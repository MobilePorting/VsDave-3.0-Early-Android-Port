...
class TerminalState extends FlxState
{
	...
	var unformattedSymbols:Array<String> = [
		"PERIOD", "BACKSLASH", "ONE", "TWO", "THREE", "FOUR", "FIVE", "SIX", "SEVEN", "EIGHT", "NINE", "ZERO", "PLUS", "COMMA", "QUESTION", "HASHTAG"
	];

	var formattedSymbols:Array<String> = [".", "/", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "+", ",", "?", "#"];

	var badKeys:Array<FlxKey> = [
		FlxKey.ESCAPE,
		FlxKey.ALT,
		FlxKey.CONTROL,
		FlxKey.SHIFT,
	...
	function expungedReignStarts()
	{
		...
		for (i in 0...amountofText)
		{
			new FlxTimer().start(1, function(timer:FlxTimer)
			{
				...
				var fakeDisplay:FlxText = new FlxText(0, i * (displayText.height), FlxG.width,
					"> " + expungedLines[new FlxRandom().int(0, expungedLines.length - 1)], 19);
				add(fakeDisplay);
				FlxG.camera.follow(fakeDisplay, 1);
			});
		}

		var glitch = new BGSprite('glitch', 0, 0, Paths.image('glitch3', 'shared'), [new Animation('glitchScreen', 'glitch 3', 15, true, [false, false])], 0,
			0, true);
